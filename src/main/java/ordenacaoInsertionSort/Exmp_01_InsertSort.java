...
public class Exmp_01_InsertSort {
    ...
    public static void main(String[] args) {
        ...
        for (vAux = 0; vAux < TAM; vAux++) {
            vVetor[vAux] = random.nextInt(100); // Preenche o vetor
// com valores aleatórios;
    ...
    private static void fInsertion_Sort(int[] pVetor) {
        ...
        int vTemp;
        int vTroca;
        for (vAux = 1; vAux < TAM; vAux++) // vAux começa na posição 1 do
        // vetor e vai até a ultima
        // posição;
        {
            vTemp = vAux; // vTemp recebe a posição que está passando no
            // "for";
            while (pVetor[vTemp] < pVetor[vTemp - 1]) // Enquanto o valor
            //que está passando na posição "vTemp" for menor que a
